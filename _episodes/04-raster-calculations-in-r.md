...
We get an error: `Error in compareRaster(x) : different extent`. We must first change the extents of the two rasters to match if we are to use `overlay()`:
...
~~~
extent(DTM_HARV) <- extent(DSM_HARV)
#check new extent
extent(DTM_HARV)
~~~
{: .language-r}
