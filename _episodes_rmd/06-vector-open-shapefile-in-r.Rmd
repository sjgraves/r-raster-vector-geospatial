...
aoi_boundary_HARV <- st_read(
  "data/vector/HarClip_UTMZ18.shp")
```
...
Remembering from an earlier lesson that CRS information uses a mathematical model  to connect data to the Earth’s surface.

When we read in the shapefile into an R object, the metadata included the CRS – in this case is 32618
We can get more information about the CRS with the st_crs() function, and the proj4string method to get information on the CRS in the proj4string.   
...
> > ```
> > 
> > Then we check the geometry type of each object: 
> > ```{r}
> > st_geometry_type(lines_HARV)
> > st_geometry_type(point_HARV)
> > ```
> > We see that the lines_HARV has a number of multilinestring geometry items, and the point_HARV item has a point item.
>
