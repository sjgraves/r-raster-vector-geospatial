...
lines_HARV <- st_read("data/vector/HARV_roads.shp")
plots_HARV <- st_read("data/vector/PlotLocations_HARV.shp")
```
...
a data frame in order to plot using `ggplot`.


For this plot, I am going to do a little trick to add the extent of the CHM to our plot, without creating a spatial extent object and without including the actual CHM in the plot. To get the spatial extent, I am going to use the `st_bbox()` function. This stands for bounding box, and the function will extract the 4 corners of the rectangle that encompass all the features contained in this object. I will also use the function `st_as_sfc` that we saw before to convert the extent into a spatial object that we can include in our plot. All of this will happen within the `geom_sf` function in our `ggplot` code. 
